postgres:
	docker run --name postgres14 --network multitenant-network -p 5433:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=secret -d postgres:14.3-alpine
...
mock:
	mockgen -package mockdb  -destination db/mock/store.go multitenant.com/app/db/sqlc Store
